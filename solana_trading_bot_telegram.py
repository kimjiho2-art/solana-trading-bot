"""
솔라나(SOL/USDT) 실시간 자동 매매 봇 + 텔레그램 알림
        ...
        try:
            if self.position_type == 'LONG':
                if current_price <= self.stop_loss:
                    ...
                    self.current_balance += (pnl - exit_fee)
                    
                    logger.info(f"❌ 롱 손절")
                    logger.info(f"   손절가: ${self.stop_loss:.2f}")
                    logger.info(f"   손실: ${pnl - exit_fee:.2f}")
                    ...
                    msg += f"📉 손실: <b>${pnl - exit_fee:.2f}</b>\n"
                    msg += f"💰 남은 자본: ${self.current_balance:.2f}\n"
                    msg += f"⏰ 시간: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                    send_telegram_message(msg)
                    
                    self.losing_trades += 1
                    self._record_trade(pnl - exit_fee)
                    self._exit_position()
                    return True
                    ...
                    logger.info(f"   수익: ${pnl - exit_fee:.2f}")
                    logger.info(f"   남은 자본: ${self.current_balance:.2f}")
...
def main():
    logger.info("=" * 80)
    logger.info("🚀 솔라나 자동 매매 봇 시작 (텔레그램 알림 기능)")
    logger.info("=" * 80)
