# -*- coding: utf-8 -*-
"""
솔라나(SOL/USDT) 실시간 자동 매매 봇 + 텔레그램 알림 + 매매일지 + 긴급 오류 감지
...
class SolanaBot:
    def __init__(self, client, symbol, initial_balance, leverage, error_detector):
        self.client = client
        self.symbol = symbol
...
def main():
    logger.info("=" * 80)
    logger.info("🚀 솔라나 자동 매매 봇 시작 (긴급 오류 감지 시스템 활성화)")
    logger.info("=" * 80)
    ...
    bot = SolanaBot(client, SYMBOL, INITIAL_BALANCE, LEVERAGE, error_detector)
    ...
    while True:
        try:
            ...
            if df is None:
                ...
                error_detector.log_error(f"데이터 조회 실패 ({error_count}/{max_errors})", '데이터 오류')
                time.sleep(5)
                continue
